...
void RasterizerQuad::clearBuffers() {
	...
	// Set all colors in pixelBuffer to be 0
	for (int i = 0; i < (width*height*3); i++) {
		pixelBuffer[i] = 0;
	}

	// Set all values in z-buffer to be negative infinity (farthest)
...
void RasterizerQuad::rasterize(glm::mat4 M, glm::mat4 V, glm::mat4 P, glm::mat4 D, PointCloud& obj) {
	...
	for (int i = 0; i < points.size(); i++) {
		glm::vec4 result =  D * P * V * M * glm::vec4(points[i], 1.0f);
		result /= result[3];
		glm::vec3 color = 0.5f * glm::normalize(normals[i]) + 0.5f;

		for (int i = 0; i < obj.getPointSize(); i++) {
			for (int j = 0; j < obj.getPointSize(); j++) {
				drawPoint((int)result[0]+i, (int)result[1]+j, result[2], color[0], color[1], color[2]);
			}
		}
	}
	displayResult();
